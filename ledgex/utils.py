...
def data_from_json_store(data_store: str, filter: list = []) -> Dict:
    ...
    for account in filter:
        filter_accounts = filter_accounts + [account] + orig_account_tree.get_descendents(account)
...
def make_bar(trans: pd.DataFrame,
    ...
    if deep:
        tba = trans[trans[CONST['account_col']].isin([account_id] + account_tree.get_descendents(account_id))]
    else:
